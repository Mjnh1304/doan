...
<div class="max-w-4xl mx-auto p-6 bg-white rounded-xl shadow-md">
    ...
    {{-- Thông tin villa --}}
    <h2 class="text-3xl font-bold mb-4 text-indigo-700">{{ $villa->name }}</h2>
    @if($villa->reviews->count())
    <p class="text-yellow-500 text-sm mb-4">
        ★
        <span class="text-gray-500">{{ number_format($villa->reviews->avg('rating'), 1) }} / 5 ({{ $villa->reviews->count() }} {{ __('booking.rate') }})</span>
    </p>
    @endif
    <div class="mb-6">
    ...
    <div class="flex justify-center">
        <div class="w-full md:w-2/3 lg:w-1/2">
            <form action="{{ route('bookings.store') }}" method="POST"
                ...
                @csrf
                <input type="hidden" name="bookable_id" value="{{ $villa->id }}">
                <input type="hidden" name="bookable_type" value="App\Models\Villa">
    ...
    <div class="mt-10">
        ...
        <form action="{{ route('reviews.store') }}" method="POST" class="space-y-4 mb-6">
            @csrf
            <input type="hidden" name="reviewable_id" value="{{ $villa->id }}">
            <input type="hidden" name="reviewable_type" value="App\Models\Villa">
            <div>
        ...
        @if ($villa->reviews()->whereNull('parent_id')->count() > 5)
    <div class="text-center mt-4">
        <button onclick="loadMoreReviews({{$villa->id}})"
...
@endif
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                try {
                    const bookedDatesObj = @json($bookedDates[$villa -> id] ?? []);
...
function loadMoreReviews(villaId) {
    fetch(`/villas/${villaId}/reviews/load?offset=${reviewOffset}`)
        .then(res => res.json())
        .then(data => {
            if (data.html.trim() === '') {
